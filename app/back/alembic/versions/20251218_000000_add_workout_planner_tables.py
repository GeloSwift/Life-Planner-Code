...
def upgrade() -> None:
    ...
    # Helper pour vérifier si un enum existe
    ...
    if not enum_exists('activitytype'):
        op.execute("""
    ...
    if not enum_exists('musclegroup'):
        op.execute("""
    ...
    if not enum_exists('sessionstatus'):
        op.execute("""
    ...
    # Définir les enums pour les colonnes (create_type=False car déjà créés)
    activitytype_enum = postgresql.ENUM(
        'musculation', 'course', 'cyclisme', 'natation', 'volleyball',
        'boxe', 'basketball', 'football', 'tennis', 'yoga', 'crossfit',
        'hiit', 'danse', 'autre',
        name='activitytype',
    ...
    if not table_exists('workout_session_exercises'):
        op.create_table(
    ...
    if not table_exists('workout_sets'):
        op.create_table(
            'workout_sets',
            sa.Column('id', sa.Integer(), nullable=False),
