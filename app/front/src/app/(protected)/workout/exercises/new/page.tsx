...
export default function NewExercisePage() {
  ...
  const handleAddField = async () => {
    ...
    try {
      ...
      loadActivityTypes(); // S'assurer que l'ID est correct
    } catch (err) {
      loadActivityTypes(); // Rollback
  ...
  const handleSubmit = async () => {
    ...
    success(`Exercice "${payloadName}" en cours de création...`);
    ...
    try {
      ...
      success(`Exercice "${payloadName}" créé !`);
    } catch (err) {
      showError(err instanceof Error ? err.message : "Erreur lors de la création");
      // On ne peut pas facilement annuler la redirection ici, 
      // mais on informe l'utilisateur par toast
    }
  };
